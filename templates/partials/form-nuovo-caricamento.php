 ...
 * usato da admin/nuovo-caricamento.php.
...
<form method="post" action="<?= htmlspecialchars($action) ?>" enctype="multipart/form-data" class="flex flex-col gap-4 form-nuovo-caricamento" id="<?= htmlspecialchars($formId) ?>">
    ...
    <div>
        ...
        <select name="tipo_documento" class="select select-bordered w-full tipo-documento-select" required>
            <option value="busta_paga" selected>Busta paga</option>
            <option value="cu">CU</option>
        </select>
    </div>

    <div class="campi-busta-paga">
        ...
        <select name="mese" class="select select-bordered w-full">
            <?php for ($m = 1; $m <= 12; $m++): ?>
                <option value="<?= $m ?>" <?= $m === (int) date('n') ? 'selected' : '' ?>><?= formatMese($m) ?></option>
            <?php endfor; ?>
        </select>
    </div>
...
<div class="overlay-elaborazione fixed inset-0 bg-base-100/80 z-50 items-center justify-center gap-3 flex-col" style="display: none">
    <span class="loading loading-spinner loading-lg text-primary"></span>
    <p class="text-sm font-medium">Caricamento ed elaborazione del PDF in corso...</p>
    <p class="text-xs text-base-content/60">Puo' richiedere qualche secondo per file con molte pagine. Non chiudere ne' ricaricare la pagina.</p>
</div>
